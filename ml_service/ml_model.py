...
DATASETS = {
    "mumbai-city": "data/mumbai-city-rera-data.csv",
    "palghar": "data/palghar-rera-data.csv",
    "raigarh": "data/raigarh-rera-data.csv",
    "dadra-daman": "data/dadra-daman-rera-data.csv",
    "mumbai-suburban": "data/mumbai-suburban-rera-data.csv",
    "nagpur": "data/nagpur-rera-data.csv",
    "nashik": "data/nashik-rera-data.csv",
    "pune": "data/pune-rera-data.csv",
    "thane": "data/thane-rera-data.csv",
    "revoked-projects": "data/revoked-projects-rera-data.csv"
}
...
# API for fetching filtered RERA dataset
@app.route("/rera-data", methods=["GET"])
def get_rera_data():
    try:
        region = request.args.get("region")
        if not region or region not in DATASETS:
            return jsonify({"error": "Invalid or missing region parameter"}), 400
        
        df = pd.read_csv(DATASETS[region])
